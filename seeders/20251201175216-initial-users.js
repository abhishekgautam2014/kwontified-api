...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    const users = accounts.map(acc => ({
      ...
      username: acc.account_name.replace(/\s+/g, '').toLowerCase(),
      email: `${acc.account_name.replace(/\s+/g, '').toLowerCase()}@example.com`,
      name: acc.account_name,
      description: `Description for ${acc.account_name}`,
      password: hash,
      createdAt: new Date(),
      updatedAt: new Date()
    }));
